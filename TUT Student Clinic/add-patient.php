...
if(isset($_POST['submit']))
{
    if(isset($_POST['name'],$_POST['surname'],$_POST['email'],$_POST['password']) && !empty($_POST['name']) && !empty($_POST['surname']) && !empty($_POST['email']) && !empty($_POST['password']))
    {
        ...
        if(filter_var($email, FILTER_VALIDATE_EMAIL))
		{
            ...
            if($stmt->rowCount() == 0)
            {																																															
                $sql = "insert into tbluser (name, surname, cellphone, altNumber, email, `password`, 
				idNum, dob, gender, address, country, married, city, pcode, province, created_at,updated_at,
				role, age, date, modeArrival, accompanied, currentComp, allergies, pulse, bp, patientName, returnCondition,
				medPrescription, returnDate  ) 
				values(:name,:surname,:cellphone,:altNumber, :email, :password, :idNum, :dob, :gender, :address, :country, :married, :city,
				:pcode, :province, :created_at,:updated_at, :role, :age, :date, :modeArrival, :accompanied, :currentComp, :allergies, :pulse, :bp, :patientName, :returnCondition,
				:medPrescription, :returnDate )";
